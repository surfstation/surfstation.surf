...
if r.status_code == 200:
  ...
  result['buoy']['swell_height_feet'] = re.match(r'.*<strong>Significant Wave Height:</strong>\s+(\d+\.\d+)\s+ft', data, re.DOTALL | re.IGNORECASE)[1]
  result['buoy']['swell_period_secs'] = re.match(r'.*<strong>Average Period:</strong>\s+(\d+\.\d+)\s+sec', data, re.DOTALL | re.IGNORECASE)[1]
  result['buoy']['swell_angle_degrees'] = re.match(r'.*<strong>Mean Wave Direction:</strong>.*\((\d+)&#176;\)', data, re.DOTALL | re.IGNORECASE)[1]
  result['buoy']['temperature_fahrenheit'] = re.match(r'.*<strong>Water Temperature:</strong>\s+(\d+\.\d+)&#176;F\s+', data, re.DOTALL | re.IGNORECASE)[1]
